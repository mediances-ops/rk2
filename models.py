# DOC-OS VERSION : V.62 SUPRÊME RADICAL
...
class Reperage(Base):
    ...
    id = Column(Integer, primary_key=True); token = Column(String(32), unique=True); statut = Column(String(20), default='brouillon'); progression_pourcent = Column(Integer, default=0)
    ...
    # Placeholders Gardien/Lieu pour éviter l'ImportError dans app.py
    gardiens = relationship("Gardien", backref="reperage")
    lieux = relationship("Lieu", backref="reperage")
...
def get_session(engine): return sessionmaker(bind=engine)()# DOC-OS VERSION : V.61 SUPRÊME MISSION CONTROL
...
class Reperage(Base):
    ...
    # 100 COLONNES FLAT-DATA (SUBSTANCE)
    ville = Column(String(255)); population = Column(String(255)); langues = Column(String(255)); climat = Column(String(255)); histoire = Column(Text); traditions = Column(Text); acces = Column(Text); hebergement = Column(Text)
    fete_nom = Column(String(255)); contraintes = Column(Text); arc = Column(Text); moments = Column(Text); sensibles = Column(Text); budget = Column(String(100)); notes = Column(Text)
