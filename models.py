...
def init_db(database_url=None):
    ...
    - Accepte database_url en paramètre (priorité absolue)
    - Sinon utilise DATABASE_URL depuis environnement (Railway)
    - Fallback sur SQLite en local
    """
    if database_url is None:
        # Essayer DATABASE_URL de l'environnement (Railway/Heroku)
        database_url = os.environ.get('DATABASE_URL')
        
        if database_url:
            # ✅ Railway utilise postgres:// mais SQLAlchemy 1.4+ veut postgresql://
            if database_url.startswith('postgres://'):
                database_url = database_url.replace('postgres://', 'postgresql://', 1)
            print(f"✅ Connexion à PostgreSQL (Railway)")
        else:
            # Fallback sur SQLite pour développement local
            database_url = 'sqlite:///reperage.db'
            print(f"📊 Base de données: SQLite (reperage.db)")
    else:
        # URL passée explicitement en paramètre
        if database_url.startswith('postgres://'):
            database_url = database_url.replace('postgres://', 'postgresql://', 1)
        print(f"✅ Connexion à base de données personnalisée")
    
    # Créer le moteur avec pool_pre_ping pour PostgreSQL
    engine_args = {'echo': False}
    
    # Configuration spécifique PostgreSQL pour éviter les déconnexions
    ...
    print(f"✅ Tables créées/vérifiées avec succès")
