...
# STATUS : DETERMINISTIC TO_DICT (FIX PERSISTENCE & DATA LOSS)
...
class Reperage(Base):
    ...
    def to_dict(self):
        """SOUDURE DE FER : Renvoie les 100 clés pour garantir l'affichage HTML."""
        # On ne nettoie plus agressivement pour éviter de vider le formulaire
        pairs = {}
        for i in [1, 2, 3]:
        ...
        # Dictionnaire racine exhaustif
        res = {
            "id": self.id, "token": self.token, "statut": self.statut, 
            "region": self.region, "pays": self.pays, "fixer_nom": self.fixer_nom,
            "image_region": self.image_region, "notes_admin": self.notes_admin,
            ...
            "fete_origines": self.fete_origines, "fete_visuel": self.fete_visuel,
            "fete_deroulement": self.fete_deroulement, "fete_responsable": self.fete_responsable,
...
class Gardien(Base):
    ...
    def to_dict(self, prefix=""): 
        return {f"{prefix}{c.name}": getattr(self, c.name) for c in self.__table__.columns if c.name not in ['id', 'reperage_id', 'index']}
