...
# Migration auto pour progression
...
def admin_dashboard():
    ...
    try:
        ...
        # Fix 10 : Filtres actifs Dashboard
        statut_f = request.args.get('statut')
        pays_f = request.args.get('pays')
        if statut_f:
            query = query.filter(Reperage.statut == statut_f)
        if pays_f:
            query = query.filter(Reperage.pays == pays_f)
...
def admin_fixers_list():
    ...
    try:
        ...
        if search:
            query = query.filter(or_(Fixer.nom.like(f"%{search}%"), Fixer.societe.like(f"%{search}%")))
        if pays:
...
def admin_edit_fixer(id=None):
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            # SAUVEGARDE INTÉGRALE FIXER (Fix 4 & 5)
            fields = ['nom', 'prenom', 'email', 'telephone', 'telephone_2', 'societe', 'fonction', 
...
# =================================================================
# II. GESTION DES REPÉRAGES (FIX 1, 7 & SUPPRESSION)
# =================================================================
...
# =================================================================
# III. IMPRESSION HAUTE SUBSTANCE (FIX 7 - 7 POINTS)
# =================================================================
...
def admin_print_high_sub(id):
    ...
    try:
        ...
        # Couplage G+L pour le template print
        pairs = []
        for i in [1, 2, 3]:
...
def admin_zip_media(id):
    ...
    memory_file.seek(0)
    return send_file(memory_file, download_name=f"Substance_Photos_Rep_{id}.zip", as_attachment=True)

# =================================================================
# IV. API SOUDURE FRONT-END (SYNC INTÉGRALE)
# =================================================================
...
def api_sync_high_sub(id):
    ...
    try:
        ...
        # Sauvegarde segmentée Gardiens & Lieux (Fix 2)
        if 'gardiens' in data:
...
def route_form_dist(token):
    ...
    try:
        ...
        f_data = {
            ...
            'nom': f.nom if f else '', 'prenom': f.prenom if f else '', 'langue_default': f.langue_preferee if f else 'FR'
        }
...
def admin_create_new_rep():
    ...
    try:
        data = request.json
        new_rep = Reperage(token=secrets.token_urlsafe(16), region=data.get('region'), pays=data.get('pays'), fixer_id=data.get('fixer_id'), fixer_nom=data.get('fixer_nom'), statut='brouillon')
        session.add(new_rep)
        session.commit()
        return jsonify({'status': 'success'})
    finally:
...
def serve_uploads_folder(rep_id, filename):
