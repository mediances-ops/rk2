# DOC-OS VERSION : V.68 SUPRÊME MISSION CONTROL
# ENGINE : FLASK + SQLALCHEMY SCOPED SESSIONS
# ÉTAT : HARMONIZED - CHAT READ STATUS & ISO DATES ACTIVATED
...
def admin_dashboard():
    ...
    for r in reps:
        ...
        # Compte des messages non lus provenant du correspondant
        unread = session.query(Message).filter_by(reperage_id=r.id, auteur_type='fixer', lu=False).count()
        d = r.to_dict()
        d['unread_count'] = unread # Injection directe pour le HTML
...
def admin_fixers_list():
    session = get_db(); query = session.query(Fixer)
    search = request.args.get('search'); pays = request.args.get('pays'); langue = request.args.get('langue')
...
# --- MOTEUR DE SYNCHRONISATION (100 CHAMPS) ---
...
def api_chat(id):
    ...
    if request.method == 'GET':
        ...
        # TRACEABILITÉ DU CHEMIN : Si l'admin consulte, on marque les messages du fixer comme "lus"
...
def route_form_fixer(token):
    ...
    if not rep: abort(404)
    d = rep.to_dict(); d['image_region'] = rep.image_region # TRACEABILITÉ IMAGE
