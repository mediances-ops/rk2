...
@app.route('/admin/reperage//print')
@nocache
def admin_print(id):
...
@app.route('/admin/reperage//update', methods=['PUT'])
@nocache
...
@app.route('/admin/reperage//delete', methods=['DELETE'])
@nocache
...
@app.route('/api/reperages/', methods=['GET', 'PUT'])
@nocache
...
val = data.get(f"lieu{i}{f}")
if val is not None: setattr(l_obj, f, str(val).strip())
session.commit(); return jsonify({'status': 'success'})

@app.route('/api/reperages//submit', methods=['POST'])
@nocache
def api_submit(id):
...
@app.route('/api/reperages//medias', methods=['GET', 'POST'])
@nocache
def api_medias(id):
session = get_db()
...
@app.route('/api/reperages//messages', methods=['GET', 'POST'])
@nocache
def api_chat(id):
session = get_db()
if request.method == 'GET':
