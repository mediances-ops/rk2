# DOC-OS VERSION : V.73.0 SUPRÊME MISSION CONTROL
# ÉTAT : CRITICAL STABILITY - EXPLICIT MAPPING (SOUDURE DE FER)
...
# --- MOTEUR DE SOUDURE DÉTERMINISTE (100 CHAMPS) ---
...
def api_sync_engine(id):
    ...
    # 1. RAIL TERRITOIRE (Affectation forcée)
    territoire_fields = ['villes', 'population', 'langues', 'climat', 'histoire', 'traditions', 'acces', 'hebergement', 'contraintes', 'arc', 'moments', 'sensibles', 'budget', 'notes']
    for field in territoire_fields:
        if field in data:
            setattr(rep, field, str(data[field]).strip())

    # 2. RAIL FESTIVITÉ
    fete_fields = ['fete_nom', 'fete_date', 'fete_gps_lat', 'fete_gps_long', 'fete_origines', 'fete_visuel', 'fete_deroulement', 'fete_responsable']
    for field in fete_fields:
        if field in data:
            setattr(rep, field, str(data[field]).strip())

    # 3. RAILS GARDIENS & LIEUX (Relational Forge)
    for i in [1, 2, 3]:
        # Gardien i
        g_obj = session.query(Gardien).filter_by(reperage_id=rep.id, index=i).first()
        if not g_obj: 
            g_obj = Gardien(reperage_id=rep.id, index=i); session.add(g_obj)
        ...
        if not l_obj:
            l_obj = Lieu(reperage_id=rep.id, index=i); session.add(l_obj)
        
        l_map = ['nom', 'type', 'gps_lat', 'gps_long', 'description', 'cinegenie', 'axes', 'points_vue', 'moments', 'son', 'acces', 'securite', 'elec', 'espace', 'meteo', 'permis']
        for field in l_map:
...
# --- AUTRES ROUTES MAINTENUES INTÉGRALEMENT ---
@app.route('/admin')
def admin_dashboard():
