# DOC-OS VERSION : V.70.8 SUPRÊME MISSION CONTROL
# ÉTAT : STABLE - FIX 415/404 API & BANNER VISIBILITY
...
# --- API SYNC DYNAMIQUE (FIXED 403/415) ---
...
def api_sync_engine(id):
    ...
    # TRACABILITÉ : On autorise le GET pour tout le monde (Consultation)
    if request.method == 'GET':
        return jsonify(rep.to_dict())

    # On bloque le PUT seulement si le dossier n'est plus en brouillon
...
def route_form_fixer(token):
    ...
    if not rep: abort(404)
    # TRACABILITÉ : On envoie l'objet to_dict() complet à index.html
