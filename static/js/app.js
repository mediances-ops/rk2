/**
 * DOC-OS V.32 - CERVEAU DE SYNCHRONISATION
...
// MODULE i18n RECURSIF (SOUDURE 3)
...
// CALCULATEUR DYNAMIQUE (SYNC 112)
function calculateProgress() {
    ...
    if (statsEl) statsEl.innerHTML = `<span id="progress-filled">${filled}</span> champs remplis sur ${total}`;
    return percent;
}

// CHARGEMENT & REMPLISSAGE RECURSIF
...
// SAUVEGARDE & SEGMENTATION (SOUDURE 1 & 2)
async function saveReperage(notif) {
    ...
    if (res.ok && notif) alert("✅ Substance Synchronisée (" + p + "%)");
}
...
// ... (initFileUpload et loadMedias identiques V30)
function initLanguageSelector() {
    document.querySelectorAll('.lang-btn').forEach(btn => {
        btn.onclick = () => { document.querySelectorAll('.lang-btn').forEach(b => b.classList.remove('active')); btn.classList.add('active'); loadTranslations(btn.dataset.lang); };
    });
}
function initForms() {
